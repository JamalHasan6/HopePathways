...
exports.hasCrisisSignal = hasCrisisSignal;
exports.classifyWithAI = classifyWithAI;
exports.generateFollowUpWithAI = generateFollowUpWithAI;
exports.generateNextQuestionWithAI = generateNextQuestionWithAI;
...
function fallbackFollowUpQuestion(input) {
...
function fallbackNextQuestion(input) {
    const questionBank = [
...
function ruleBasedClassify(transcript) {
    if (hasCrisisSignal(transcript))
        return "crisis";
    const lower = transcript.toLowerCase();
    if (/community|peer|group|connection|lived experience|not ready for clinic/.test(lower))
        return "peer_support";
    if (/information|tips|coping|resources|just want info|self help/.test(lower) &&
        !/struggling|not coping|overwhelmed|exhausted|hopeless/.test(lower))
        return "self_guided";
    return "hub_referral";
}
function ruleBasedFlags(transcript) {
    const flags = [];
    const lower = transcript.toLowerCase();
    if (/first time|never spoken|not before|new to this/.test(lower))
        flags.push("first_time_seeker");
    if (/alone|no one|isolated|by myself/.test(lower))
        flags.push("social_isolation");
    if (/emergency|hospital|ed |ambulance/.test(lower))
        flags.push("previous_ed_presentation");
    return flags;
}
function ruleBasedResource(c) {
    if (c === "crisis")
        return "Lifeline 13 11 14 / Emergency 000";
    if (c === "peer_support")
        return "SANE Forums — sane.org";
    if (c === "self_guided")
        return "Beyond Blue — beyondblue.org.au";
    return "Hope Pathways Health & Wellbeing Hub (Mon-Fri 9am-8pm, Sat 10am-4pm, Newcastle NSW)";
}
function fallbackTriage(entries) {
...
async function classifyWithAI(entries) {
...
async function generateNextQuestionWithAI(input) {
...
function contentAwareAck(answer) {
...
function contentAwareQuestion(answer, askedSoFar) {
    const a = answer.toLowerCase();
    ...
    const isAsked = (question) => askedLower.some((msg) => msg.includes(question.toLowerCase()));
    const candidates = [
